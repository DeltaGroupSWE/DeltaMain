...
function getObjectID(mx, my) {
    ...
    //let mPageImg = image(mPage,-mPage.width/2,-mPage.height/2,mPage.width,mPage.height,-mPage.width/2,-mPage.height/2);//mPage.elt.getContext('webgl');
	let fb = createFramebuffer();
    fb.pixelDensity(1);
    fb.begin();
    
    fb.end();
    fb.loadPixels();
    //console.log(fb.pixels.length)
	var index = 4 * ((height-my) * fb.width + mx);
    console.log(fb.pixels[index]<<16 | fb.pixels[index+1]<<8 | fb.pixels[index+2]);
    return (fb.pixels[index]<<16 | fb.pixels[index+1]<<8 | fb.pixels[index+2]);
}
...
function getPixels() {
	...
	mPage.readPixels(0, 0, gl.drawingBufferWidth, gl.drawingBufferHeight, gl.RGBA, gl.UNSIGNED_BYTE, pixels);
	return (pixels);
}
